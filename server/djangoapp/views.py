...
def get_dealer_details(request, dealer_id):
    ...
    try:
        ...
        return JsonResponse({
            "status": 500,
            "message": "Error fetching dealer details"
        })


def get_dealer_reviews(request, dealer_id):
    """Fetch reviews for a dealer and analyze sentiment"""
    try:
        if dealer_id:
            ...
            for review_detail in reviews:
                try:
                    ...
                    logger.error(
                        f"Error analyzing sentiment: {err}"
                    )
...
def get_cars(request):
    ...
    try:
        ...
        traceback.print_exc()
        return JsonResponse({
            "status": 500, "message": f"Error: {str(err)}"
        })
